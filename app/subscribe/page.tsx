...
export default function SubscribePage() {
  ...
  async function activatePlan() {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : "Unable to activate subscription right now.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function buyTryOnPack() {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : "Unable to purchase try-on pack right now.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="card phone-single" style={{ maxWidth: 560, margin: "0 auto" }}>
      ...
      <div className="grid" style={{ marginTop: 10 }}>
        <div className="badge">Status: {active ? "Active" : "Inactive"}</div>
